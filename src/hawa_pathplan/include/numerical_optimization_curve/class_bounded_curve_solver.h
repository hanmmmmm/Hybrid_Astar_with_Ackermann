...
public:
    ...
    void operator()(ADvector& fg, const ADvector& x)
    {
        ...
        for(int i = 0; i < m_num_steps_; i++)
        {
            ...
            AD<double> _t0_x = _samples[_samples.size()-1][index_pose_x];
            AD<double> _t0_y = _samples[_samples.size()-1][index_pose_y];
            AD<double> _t0_yaw = _samples[_samples.size()-1][index_pose_yaw];

            std::cout << "_t0 x y yaw  " << _t0_x << " " << _t0_y << " " << _t0_yaw << std::endl;

            AD<double> _t0_v = _samples[_samples.size()-1][index_linear_v];
            AD<double> _t0_a = _samples[_samples.size()-1][index_linear_a];

            AD<double> _t0_s = _samples[_samples.size()-1][index_aglr_str_angle];
            AD<double> _t0_sr = _samples[_samples.size()-1][index_aglr_str_rate];
            ...
            AD<double> _new_v = _t0_v + _new_acc * m_step_time_sec_;
            ...
            AD<double> _new_steer_angle = _t0_s + _new_steer_rate * m_step_time_sec_;
            ...
            AD<double> _d_yaw = _new_angular_velo * m_step_time_sec_;
            ...
            {
                _body_frame_dx = _t0_v * m_step_time_sec_;
                _body_frame_dy = 0.0;
            }
            ...
            _temp[index_pose_x] = _new_x;
            _temp[index_pose_y] = _new_y;
            _temp[index_pose_yaw] = _new_yaw;
            _temp[index_linear_v] = _new_v;
            _temp[index_linear_a] = _new_acc;
            _temp[index_aglr_str_angle] = _new_steer_angle;
            _temp[index_aglr_str_rate] = _new_steer_rate;
...
public:
    ...
    void set_agent_initial_state(StructModelStates init_state);

    void set_agent_target_state(StructModelStates goal_state);

    void set_trajectory_step_number(const int num);
        ...
        )
{
    ...
    for (int sol=0; sol<_computed_control_variables.size()/2; sol++)
    {
        std::cout << _computed_control_variables[sol*2] << "  " << _computed_control_variables[sol*2+1] << std::endl;
    }
