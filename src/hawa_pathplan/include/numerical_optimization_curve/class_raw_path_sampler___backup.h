...
struct StructMotionSequence
{
    ...
    double limit_acc_incr = 0;
    double limit_acc_decr = 0;

    double actual_acc_incr = 0;
    double actual_acc_decr = 0;

    double actual_top_v = 0;

    double time_step_sec = 0;

    double total_distance = 0;

    int steps_in_up = 0;  // the amount of steps needed during the stage of increasing speed
    int steps_in_hold = 0;  // the amount of steps needed during the stage of constant speed
    int steps_in_down = 0;  // the amount of steps needed during the stage of decreasing speed
...
/// @param total_distance 
/// @return 
void get_motion_sequence_durations(StructMotionSequence& motion_seq, vector<double>& acc_str)
{
    /*
    V1:
    ...
    */
    ...
    double _guess_ramp_up_time = motion_seq.limit_velocity / motion_seq.limit_acc_incr;
    double _guess_ramp_dw_time = motion_seq.limit_velocity / motion_seq.limit_acc_decr;
    
    double _guess_ramp_up_distance = 0.5 * motion_seq.limit_acc_incr * std::pow(_guess_ramp_up_time, 2);
    double _guess_ramp_dw_distance = 0.5 * motion_seq.limit_acc_decr * std::pow(_guess_ramp_dw_time, 2) + motion_seq.limit_velocity * _guess_ramp_dw_time;

    if (motion_seq.total_distance > (_guess_ramp_up_distance + _guess_ramp_dw_distance))
    {
        _result.t_up = _complete_ramp_time;
...
void calc_motion_sequence_parameters(StructMotionSequence& motion_seq)
{
    ...
    if (motion_seq.dist_total > (_guess_acc_distance + _guess_dec_distance))
    {
        ...
        motion_seq.dist_accel = _guess_acc_distance;
        motion_seq.dist_decel = _guess_dec_distance;
        motion_seq.dist_const = motion_seq.dist_total - (_guess_acc_distance + _guess_dec_distance);

        double _guess_cnst_time = motion_seq.dist_const / motion_seq.limit_velocity;
        int _num_step_cnst = 0;
        double _ceil_cnst_time = ceil_number_to_multiply_of_base(_guess_cnst_time, _step_size, _num_step_cnst);
        motion_seq.actual_top_velocity = motion_seq.dist_const / _ceil_cnst_time;
        motion_seq.steps_in_const = _num_step_cnst;
        motion_seq.time_const = _ceil_cnst_time;

        int _num_step_acc = 0;
        _guess_acc_time = std::abs(motion_seq.actual_top_velocity / motion_seq.limit_accel);
        double _actual_acc_time = ceil_number_to_multiply_of_base(_guess_acc_time, _step_size, _num_step_acc);
        motion_seq.steps_in_accel = _num_step_acc;
        motion_seq.time_accel = _actual_acc_time;
        // motion_seq.actual_accel = std::abs(motion_seq.actual_top_velocity / motion_seq.time_accel);
        motion_seq.actual_accel = 2 * motion_seq.dist_accel / std::pow(motion_seq.time_accel, 2);

        int _num_step_dec = 0;
        _guess_dec_time = std::abs(motion_seq.actual_top_velocity / motion_seq.limit_decel);
        double _actual_dec_time = ceil_number_to_multiply_of_base(_guess_dec_time, _step_size, _num_step_dec);
        motion_seq.steps_in_decel = _num_step_dec;
        motion_seq.time_decel = _actual_dec_time;
        motion_seq.actual_decel = - std::abs(motion_seq.actual_top_velocity / motion_seq.time_decel);
    ...
    {
        double _ratio = std::abs(motion_seq.limit_decel / motion_seq.limit_accel); // k = (d_acc / d_dec) = (dec / acc)
        motion_seq.dist_decel = motion_seq.dist_total / (_ratio + 1.0);
        motion_seq.dist_accel = motion_seq.dist_total - motion_seq.time_decel;

        double _guess_acc_time = std::sqrt( 2 * motion_seq.dist_accel / motion_seq.limit_accel );
        int _num_step_acc = 0;
        double _actual_acc_time = ceil_number_to_multiply_of_base(_guess_acc_time, _step_size, _num_step_acc);
        motion_seq.steps_in_accel = _num_step_acc;
        motion_seq.time_accel = _actual_acc_time;
        ...
        motion_seq.actual_top_velocity = std::abs(motion_seq.actual_accel * motion_seq.time_accel);
        double _temp_t_decel;
        solve_quadratic_for_unique_postive_root((std::abs(motion_seq.limit_decel)/2.0),
                                                -motion_seq.actual_top_velocity,
                                                motion_seq.dist_decel,
                                                _temp_t_decel);
        int _num_step_dec = 0;
        double _actual_dec_time = ceil_number_to_multiply_of_base(_temp_t_decel, _step_size, _num_step_dec);
        motion_seq.steps_in_decel = _num_step_dec;
        motion_seq.time_decel = _actual_dec_time;
...
void calc_motion_sequence_parameters(StructMotionSequence& motion_seq)
{
    ...
    if (motion_seq.dist_total > (_guess_acc_distance + _guess_dec_distance))
    {
        ...
        // double term_a = 0.5 * t1 * t1 + t1 * t2 + t1 * t3;
        // double term_b = 0.5 * t3 * t3;

        // double slope = - term_b / term_a;
        // double intercept = motion_seq.dist_total / term_a;

        // if (intercept > motion_seq.limit_accel)
        // {
        //     std::cerr << "acceleration value looks wrong. " << std::endl;
        // }

        // double _test_a1 = slope * motion_seq.limit_decel + intercept;
        // if (_test_a1 > 0 && _test_a1 <= motion_seq.limit_accel)
        // {
        //     std::cout << "case 1. " << _test_a1 << std::endl;
        //     motion_seq.actual_accel = _test_a1;
        //     motion_seq.actual_decel = motion_seq.limit_decel;
        // }
        // else
        // {
        //     std::cout << "case 2. " << std::endl;
        //     double _test_a3 = (- term_a / term_b) * motion_seq.limit_accel + motion_seq.dist_total / term_b;

        //     motion_seq.actual_accel = motion_seq.limit_accel;
        //     motion_seq.actual_decel = _test_a3;
        // }
        motion_seq.time_accel = _actual_acc_time;
        motion_seq.time_const = _ceil_cnst_time;
        motion_seq.time_decel = _actual_dec_time;
