...
struct StructCppadInputIndices
{
    int total_num_steps = 0;  // the number of points used in the path projection.

    const int num_control = 2;  // control variables are the steer_angle and linear acceleration.

    int index_goal_x       = 0;
    int index_goal_y       = 1;
    int index_goal_yaw_sin = 2;  // in order to calculate the similarity of 2 angles, 
    int index_goal_yaw_cos = 3;  // using their sin&cos values is safer than their raw values. 
    int index_goal_v       = 4;
    int index_goal_a       = 5;

    int index_start_x      = 6;
    int index_start_y      = 7;
    int index_start_yaw    = 8;
    int index_start_v      = 9;
    int index_start_a      = 10;

    int index_control_start = 11;  // this is should be == the last index above + 1

    void set_total_num_steps(const int val)
    {
        total_num_steps = val;
    }

    int get_ith_steer_index(const int i)
    {
        return index_control_start + i;
    }

    int get_ith_accel_index(const int i)
    {
        return index_control_start + total_num_steps + i;
    }

    int get_ith_bbox_start_index(const int i)
    {
        return index_control_start + (total_num_steps*2) + (i*4);
    }

    StructCppadInputIndices(){}
};


typedef CPPAD_TESTVECTOR(double) Dvector;

class ClassCppadInputVectorGenerator
{
private:
    Dvector m_control_var_;
    Dvector m_control_var_lowerbound_;
    Dvector m_control_var_upperbound_;

    StructCppadInputIndices m_indices_;

public:
    ClassCppadInputVectorGenerator();
    ~ClassCppadInputVectorGenerator();

    // void set_num_steps(const int num_steps);
    void set_indices( StructCppadInputIndices ind);

    void set_agent_initial_state(const StructModelStates init_state);

    void set_agent_target_state(const StructModelStates goal_state);

    void set_trajectory_step_time(const double seconds);

    void set_trajectory_corridor_bboxs(const std::vector<double> bboxs);

    // void set_trajectory_controls(const );

};

ClassCppadInputVectorGenerator::ClassCppadInputVectorGenerator()
{
}
...
// void ClassCppadInputVectorGenerator::set_num_steps(const int num_steps)
// {
//     m_control_var_.resize(num_steps);
//     m_control_var_lowerbound_.resize(num_steps);

//     m_control_var_upperbound_.resize(num_steps);
// }

void ClassCppadInputVectorGenerator::set_indices( StructCppadInputIndices ind)
{
    memcpy(&m_indices_, &ind, sizeof(ind));
    int num_steps = m_indices_.total_num_steps;
    m_control_var_.resize(num_steps);
    m_control_var_lowerbound_.resize(num_steps);
    m_control_var_upperbound_.resize(num_steps);
}

void ClassCppadInputVectorGenerator::set_agent_initial_state(const StructModelStates init_state)
{
    m_control_var_[m_indices_.index_start_x] = init_state.x;
    m_control_var_[m_indices_.index_start_y] = init_state.y;
    m_control_var_[m_indices_.index_start_yaw] = init_state.yaw;
    m_control_var_[m_indices_.index_start_a] = init_state.v;
    m_control_var_[m_indices_.index_start_v] = init_state.a;

    m_control_var_lowerbound_[m_indices_.index_start_x] = init_state.x;
    m_control_var_lowerbound_[m_indices_.index_start_y] = init_state.y;
    m_control_var_lowerbound_[m_indices_.index_start_yaw] = init_state.yaw;
    m_control_var_lowerbound_[m_indices_.index_start_a] = init_state.v;
    m_control_var_lowerbound_[m_indices_.index_start_v] = init_state.a;

    m_control_var_upperbound_[m_indices_.index_start_x] = init_state.x;
    m_control_var_upperbound_[m_indices_.index_start_y] = init_state.y;
    m_control_var_upperbound_[m_indices_.index_start_yaw] = init_state.yaw;
    m_control_var_upperbound_[m_indices_.index_start_a] = init_state.v;
    m_control_var_upperbound_[m_indices_.index_start_v] = init_state.a;
}

void ClassCppadInputVectorGenerator::set_agent_target_state(const StructModelStates goal_state)
{
    m_control_var_[m_indices_.index_goal_x] = goal_state.x;
    m_control_var_[m_indices_.index_goal_y] = goal_state.y;
    m_control_var_[m_indices_.index_goal_yaw_sin] = std::sin(goal_state.yaw);
    m_control_var_[m_indices_.index_goal_yaw_cos] = std::cos(goal_state.yaw);
    m_control_var_[m_indices_.index_goal_a] = goal_state.a;
    m_control_var_[m_indices_.index_goal_v] = goal_state.v;
    
    m_control_var_lowerbound_[m_indices_.index_goal_x] = goal_state.x;
    m_control_var_lowerbound_[m_indices_.index_goal_y] = goal_state.y;
    m_control_var_lowerbound_[m_indices_.index_goal_yaw_sin] = std::sin(goal_state.yaw);
    m_control_var_lowerbound_[m_indices_.index_goal_yaw_cos] = std::cos(goal_state.yaw);
    m_control_var_lowerbound_[m_indices_.index_goal_a] = goal_state.a;
    m_control_var_lowerbound_[m_indices_.index_goal_v] = goal_state.v;
    
    m_control_var_upperbound_[m_indices_.index_goal_x] = goal_state.x;
    m_control_var_upperbound_[m_indices_.index_goal_y] = goal_state.y;
    m_control_var_upperbound_[m_indices_.index_goal_yaw_sin] = std::sin(goal_state.yaw);
    m_control_var_upperbound_[m_indices_.index_goal_yaw_cos] = std::cos(goal_state.yaw);
    m_control_var_upperbound_[m_indices_.index_goal_a] = goal_state.a;
    m_control_var_upperbound_[m_indices_.index_goal_v] = goal_state.v;
    
}


void ClassCppadInputVectorGenerator::set_trajectory_corridor_bboxs(const std::vector<double> bboxs)
{
    assert(bboxs.size() != m_indices_.total_num_steps);

    for(int i=0; i<m_indices_.total_num_steps; i++)
    {
        for(int j=0; j<4; j++)
        {
            m_control_var_[m_indices_.get_ith_bbox_start_index(i)+j] = bboxs[i*4 + j];
            m_control_var_lowerbound_[m_indices_.get_ith_bbox_start_index(i)+j] = bboxs[i*4 + j];
            m_control_var_upperbound_[m_indices_.get_ith_bbox_start_index(i)+j] = bboxs[i*4 + j];
        }
    }
